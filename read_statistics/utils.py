...
def read_statistics_once_read(request, obj):
	...
	if not request.COOKIES.get(key):
		...
		readDetail, created = ReadDetail.objects.get_or_create(content_type=ct,
...
def get_today_hot_read_data(content_type):
	...
	read_details.order_by('-read_num')
...
def get_yesterday_hot_read_data(content_type):
	...
	readdetails = ReadDetail.objects.filter(content_type=content_type, date=yesterday)
	readdetails.order_by('-read_num')
